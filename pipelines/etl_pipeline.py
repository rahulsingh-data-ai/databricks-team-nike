...
dbutils.widgets.text("bronze_schema", "development.dev_gps_research_insights")
dbutils.widgets.text("target_schema", "development.dev_gps_research_insights")
BRONZE = dbutils.widgets.get("bronze_schema")
TARGET = dbutils.widgets.get("target_schema")
print(f"BRONZE={BRONZE}  TARGET={TARGET}")
...
dbutils.widgets.text("vs_index", "")  # leave blank to skip; set to e.g. development.dev_gps_research_insights.facilities_vs_index
VS_INDEX = dbutils.widgets.get("vs_index").strip()

if not VS_INDEX:
    print("Vector index sync skipped (vs_index widget is empty).")
else:
    try:
        resp = requests.post(
